# Assignment: Introduction to R (Github)
# Name: Sarousi, Erez
# Date: 2010-09-06
...
names(week1_sleep) <- names(days)
names(week2_sleep) <- names(days)
...
x <- c(student01, student02, student03)
y <- (66.3:100.0)
...
colnames(Assignments) <- c("Assignment 1", "Assignment 2", "Assignment 3")
rownames(Students) <- c("Florinda Baird", "Jinny Foss", "Lou Purvis", "Nola Maloney")

## Total points for each assignment using `colSums()`
colSums("Assignment 1")
colSums("Assignment 2")
colSums("Assignment 3")
colSums("assignment04")

## Total points for each student using `rowSums()`
rowSums("Florinda Baird")
rowSums("Jinny Foss")
rowSums("Lou Purvis")
rowSums("Nola Maloney")
...
sorted_characters_df <- characters_df[order(1:3),]
...
ringbearers_df <- characters_df[characters_df$TRUE == 1:3,]
